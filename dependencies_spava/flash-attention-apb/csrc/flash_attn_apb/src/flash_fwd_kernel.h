...
inline __device__ void compute_attn_1rowblock(const Params &params, const int bidb, const int bidh, const int m_block) {
    ...
    // printf("m_block * kBlockM: %d\n", m_block * kBlockM);

    const BlockInfo</*Varlen=*/!Is_even_MN> binfo(params, bidb);
    if (m_block * kBlockM >= binfo.actual_seqlen_q) return;

    const int n_block_min = !Is_local ? ((0 + (m_block * kBlockM < params.anchor_len ? params.prv_len : 0)) / kBlockN) : std::max(0, (m_block * kBlockM + binfo.actual_seqlen_k - binfo.actual_seqlen_q - params.window_size_left) / kBlockN);
    ...
    // printf("m_block * kBlockM: %d, n_block_min: %d, n_block_max: %d, is_local: %d, seqlen_q: %d, seqlen_k: %d, window_size_left: %d\n", m_block * kBlockM, n_block_min, n_block_max, Is_local, binfo.actual_seqlen_q, binfo.actual_seqlen_k, params.window_size_left);
...
inline __device__ void compute_attn_1rowblock_splitkv(const Params &params, const int bidb, const int bidh, const int m_block, const int n_split_idx, const int num_n_splits) {
    ...
    const int n_block_min = !Is_local
        ? std::max(n_split_idx * n_blocks_per_split, (m_block * kBlockM < params.anchor_len ? params.prv_len : 0) / kBlockN)
