...
def zigzag_lring_flash_attn_forward(
    process_group,
    q: torch.Tensor,
    k: torch.Tensor,
    k_nope: torch.Tensor,
    ...
    # nvtx.range_push(f"rank{rank}: b1_indices")
    ...
    # nvtx.range_push(f"rank{rank}: b2_indices")
    ...
    # nvtx.range_push(f"rank{rank}: query_attn")
    ...
    block_out_q, _, _, _, _, block_lse_q, _, _ = _flash_attn_forward(  
        ...
        k=query_k,
        v=query_v,
        # k=(k if rank == 0 else k[:, anchor_len:-query_len]),
        # v=(v if rank == 0 else v[:, anchor_len:-query_len]),
        dropout_p=0,
    ...
    # out_gather = [torch.zeros_like(block_out_q) for _ in range(dist.get_world_size(group=process_group))]
    # lse_gather = [torch.zeros_like(block_lse_q) for _ in range(dist.get_world_size(group=process_group))]
    ...
    # do the first block when communicating the second block
    # nvtx.range_push(f"rank{rank}: b1_attn")
    prv1 = _comm1.data()
    ...
    # nvtx.range_push(f"rank{rank}: b2_attn")
    # do the second block
    prv2 = _comm2.data()
    prv2 = prv2[::-1]
    prv2 = prv1 + prv2
    ...
    req_q_lse.wait()
    req_q_out.wait()
    # nvtx.range_push(f"rank{rank}: query_merge")
    out_q = None
    lse_q = None
...
def zigzag_lring_flash_attn_forward_no_overlap(
    process_group,
    q: torch.Tensor,
    k: torch.Tensor,
    k_nope: torch.Tensor,
...
class ZigZagLRingFlashAttnFunc(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        q,
        ...
        k = k.contiguous()
        v = v.contiguous()
        out = zigzag_lring_flash_attn_forward(
            group,
            q,
            k,
            k_nope,
...
def zigzag_lring_flash_attn_qkvpacked_func(
    qkv,
    dropout_p=0.0,
    softmax_scale=None,
    causal=False,
    ...
    return ZigZagLRingFlashAttnFunc.apply(
        qkv[:, :, 0],
        qkv[:, :, 1],
        qkv[:, :, 2],
        dropout_p,
...
def zigzag_lring_flash_attn_kvpacked_func(
    q,
    kv,
    dropout_p=0.0,
    softmax_scale=None,
...
def zigzag_lring_flash_attn_func(
    q,
    k,
    k_nope,
    v,
