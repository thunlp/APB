...
def zigzag_lring_flash_attn_forward_no_na(
    process_group,
    q: torch.Tensor,
    k: torch.Tensor,
    k_nope: torch.Tensor,
...
# comm = RingComm(process_group)
    ...
    anchor_left = 0 if rank == 0 else anchor_len
    anchor_right = anchor_len
...
class ZigZagLRingNONAFlashAttnFunc(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        q,
        ...
        k = k.contiguous()
        v = v.contiguous()
        out = zigzag_lring_flash_attn_forward_no_na(
            group,
            q,
            k,
            k_nope,
...
def zigzag_lring_no_na_flash_attn_qkvpacked_func(
    qkv,
    dropout_p=0.0,
    softmax_scale=None,
    causal=False,
    ...
    return ZigZagLRingNONAFlashAttnFunc.apply(
        qkv[:, :, 0],
        qkv[:, :, 1],
        qkv[:, :, 2],
        dropout_p,
...
def zigzag_lring_no_na_flash_attn_kvpacked_func(
    q,
    kv,
    dropout_p=0.0,
    softmax_scale=None,
...
def zigzag_lring_no_na_flash_attn_func(
    q,
    k,
    k_nope,
    v,
