...
from lring.utils import load_lring_model_tokenizer_dist, lring_prefill_and_decode_dist, load_ulysses_model_tokenizer_dist, ulysses_prefill_and_decode_dist, load_star_model_tokenizer_dist, star_prefill_and_decode_dist, load_ring_model_tokenizer_dist, ring_prefill_and_decode_dist
    ...
    if attn_type == "vllm":
        ...
        if method == 'star':
            ...
            if test_time:
                ...
                dist.barrier()
                beg_t = time.time()
            output = star_prefill_and_decode_dist(model, input_tensors['input_ids'], position_ids, noq_len, max_new_tokens=generation_config.max_new_tokens, stop_ids=stop_ids, nn=2, np=8)
            if test_time:
                dist.barrier()
                torch.cuda.synchronize()
                end_t = time.time()
            ...
            if test_time:
                ...
                dist.barrier()
                beg_t = time.time()
            output = lring_prefill_and_decode_dist(model, input_tensors['input_ids'], position_ids, noq_len, max_new_tokens=generation_config.max_new_tokens, stop_ids=stop_ids, query_ids=query_ids, nn=2, np=8)
            if test_time:
                dist.barrier()
                torch.cuda.synchronize()
                end_t = time.time()
            ...
            if test_time:
                ...
                dist.barrier()
                beg_t = time.time()
            output = ulysses_prefill_and_decode_dist(model, input_tensors['input_ids'], position_ids, max_new_tokens=generation_config.max_new_tokens, stop_ids=stop_ids, nn=2, np=8)
            if test_time:
                dist.barrier()
                torch.cuda.synchronize()
                end_t = time.time()
            ...
            if test_time:
                ...
                dist.barrier()
                beg_t = time.time()
            output = ring_prefill_and_decode_dist(model, input_tensors['input_ids'], position_ids, max_new_tokens=generation_config.max_new_tokens, stop_ids=stop_ids, nn=2, np=8)
            if test_time:
                dist.barrier()
                torch.cuda.synchronize()
                end_t = time.time()
...
if __name__ == "__main__":
    ...
    # Model
    if method == 'lring':
        model, tok = load_lring_model_tokenizer_dist(args.model_name_or_path, args.locret_path, nn=2, np=8)
    elif method == 'star':
        model, tok = load_star_model_tokenizer_dist(args.model_name_or_path, nn=2, np=8)
    elif method == 'ulysses':
        model, tok = load_ulysses_model_tokenizer_dist(args.model_name_or_path, nn=2, np=8)
    elif method == 'ring':
        model, tok = load_ring_model_tokenizer_dist(args.model_name_or_path, nn=2, np=8)
    else:
    ...
    for data_name in data_names:
        ...
        print_on_master("=" * 50)
        print_on_master(data_name)
        print_on_master(f"Processing speed: {(tot_tok / tot_time):.2f}")
