...
def main(
    ...
    with torch.no_grad():
        for _ in tqdm(range(5)):
            output = model(input_ids, breakdown=True)
        output = model(input_ids, breakdown=True)
    ...
    # output_ids = generate(model, input_ids, max_new_tokens, stop_ids)
        
    # prefill_time, decode_time = generate(model, input_ids, max_new_tokens, stop_ids, break_down=True)
