...
def generate(model, input_ids, max_new_tokens, stop_ids, break_down=False):
    ...
    if break_down:
        ...
        prefill_tok = input_ids.shape[-1]
    output = model(input_ids, use_cache=True, num_logits_to_keep=1)
    ...
    if break_down:
        ...
        return prefill_end - prefill_beg, decode_end - prefill_end, prefill_tok, len(generated_ids)
...
def main(
    ...
    for _ in range(5):
        p_t, d_t, p_tok, t_tok = generate(model, input_ids, max_new_tokens, stop_ids, True)
    tot_p_t, tot_d_t, tot_p_tok, tot_d_tok = 0, 0, 0, 0
    for _ in range(5):
        p_t, d_t, p_tok, d_tok = generate(model, input_ids, max_new_tokens, stop_ids, True)
        tot_p_t += p_t
        tot_d_t += d_t
        tot_p_tok += p_tok
        tot_d_tok += d_tok
    
    print(f"prefill: {tot_p_tok / tot_p_t:.2f}")
    print(f"decode: {tot_d_tok / tot_d_t:.2f}")
