...
def main(
    ...
    # with torch.no_grad():
    #     for _ in tqdm(range(10)):
    #         output = model(input_ids[:, :context_len], query_ids=query_ids, position_ids=position_ids[:, :context_len], breakdown=True)
    #     output = model(input_ids[:, :context_len], query_ids=query_ids, position_ids=position_ids[:, :context_len], breakdown=True)
    ...
    # prefill_time, decode_time = lring_prefill_and_decode(model, input_ids, position_ids, context_len, max_new_tokens=max_new_tokens, stop_ids=stop_ids, break_down=True)

    # if rank == 0:
    #     print("=" * 50)
    #     print(f"prefill time (ms): {(prefill_time * 1000):.2f}")
    #     print(f"prefill time (ms): {(decode_time * 1000):.2f}")
    #     print("=" * 50)
