...
from lring.utils import load_ring_model_tokenizer_dist, ring_prefill_and_decode_dist
import torch.distributed as dist
...
def load_model(
    ...
    model, tokenizer = load_ring_model_tokenizer_dist(model_path, nn=nn, np=np)
    # if dist.get_rank() == 0:
    #     breakpoint()
    # dist.barrier()
...
def main(
    ...
    for input_sample in tqdm(pre_run_data, total=len(pre_run_data)):
        ...
        generated_ids = ring_prefill_and_decode_dist(model, input_ids, position_ids, max_new_tokens=max_new_tokens, stop_ids=stop_ids, nn=nn, np=np)
