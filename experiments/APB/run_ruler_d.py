...
def main(
    ...
    for seq_length in seq_lengths:
        ...
        if attn_type != 'dense' and attn_type != 'minf' and attn_type != 'flash':
            ...
            if num_nodes > 1:
                inference_executor += f'--rdzv_id "apb" --rdzv_backend=c10d --rdzv_endpoint={master}:{port} '
        else:
        ...
        for tidx, task in enumerate(tasks):
            ...
            if task_data_file is None or not os.path.exists(task_data_file):
                ...
                if attn_type == "minf" or attn_type == 'flash':
            ...
            task_gen_cmd = (
                ...
                f'--nn_ {num_nodes} '
                f'--np_ {nproc_per_node_seq_len} '
            )
            if attn_type == "minf" or attn_type == "flash":
                task_gen_cmd = f"CUDA_VISIBLE_DEVICES={tidx} " + task_gen_cmd + " & "
            print(f'\nRunning:\n{task_gen_cmd}')
            submit_job(task_gen_cmd, task_log_dir, 'generate_predictions.sh')

        if attn_type != "minf" and not test_time and attn_type != "flash":
...
if __name__ == '__main__':
    ...
    if args.attn_type != 'dense' and args.attn_type != 'minf' and args.attn_type != "flash":
