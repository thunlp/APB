...
from lring.utils import load_ulysses_model_tokenizer_dist, ulysses_prefill_and_decode_dist
import torch.distributed as dist
...
def load_model(
    ...
    model, tokenizer = load_ulysses_model_tokenizer_dist(model_path, nn=nn, np=np)
    # if dist.get_rank() == 0:
    #     breakpoint()
    # dist.barrier()
...
def main(
    ...
    for input_sample in tqdm(pre_run_data, total=len(pre_run_data)):
        ...
        input_ids = tokenizer(input_str, return_tensors='pt', add_special_tokens=False).to(model.device).input_ids
        position_ids = torch.arange(input_ids.shape[-1], dtype=torch.long, device=model.device).unsqueeze(0)
        generated_ids = ulysses_prefill_and_decode_dist(model, input_ids, position_ids, max_new_tokens=max_new_tokens, stop_ids=stop_ids, nn=nn, np=np)
