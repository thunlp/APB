...
from lring.utils import load_star_model_tokenizer, load_lring_model_tokenizer, star_prefill_and_decode, lring_prefill_and_decode, load_ulysses_model_tokenizer, ulysses_prefill_and_decode, load_ring_model_tokenizer, ring_prefill_and_decode
    ...
    if attn_type == "vllm":
        ...
        if method == 'star':
            ...
            if test_time:
                torch.cuda.synchronize()
                beg_t = time.time()
            output = ring_prefill_and_decode(model, input_tensors['input_ids'], position_ids, max_new_tokens=generation_config.max_new_tokens, stop_ids=stop_ids)
            if test_time:
                torch.cuda.synchronize()
                end_t = time.time()
        elif method == "lring":
...
if __name__ == "__main__":
    ...
    if method == 'lring':
        ...
        model, tok = load_ring_model_tokenizer(args.model_name_or_path)
    elif method == 'star':
