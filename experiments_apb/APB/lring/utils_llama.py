...
from .modeling_llama_lring_batch import LlamaForCausalLM as LRingLlamaForCausalLMBatch
...
def load_ulysses_model_tokenizer(path: str):
    rank = dist.get_rank()
    world_size = dist.get_world_size()
    model = UlyssesLlamaForCausalLM.from_pretrained(path, attn_implementation='flash_attention_2', torch_dtype=torch.bfloat16, device_map=torch.device(f"cuda:{rank}"))
    model.set_dist(rank, world_size)
    tokenizer = AutoTokenizer.from_pretrained(path)
    return model, tokenizer
...
def load_lring_model_tokenizer(path: str, locret_path: str, batch=False):
    rank = dist.get_rank()
    world_size = dist.get_world_size()
    model_class = LRingLlamaForCausalLM #if not batch else LRingLlamaForCausalLMBatch
...
def load_star_model_tokenizer(path: str):
    rank = dist.get_rank()
    world_size = dist.get_world_size()
    model = StarLlamaForCausalLM.from_pretrained(path, attn_implementation='flash_attention_2', torch_dtype=torch.bfloat16, device_map=torch.device(f"cuda:{rank}"))
    model.set_dist(rank, world_size)
    tokenizer = AutoTokenizer.from_pretrained(path)
    return model, tokenizer
...
@torch.no_grad()
def ring_prefill_and_decode(model, input_ids, position_ids, max_new_tokens = 12, stop_ids = [], debug=False, break_down=False):
    rank = dist.get_rank()
    world_size = dist.get_world_size()
    generated_ids = []
    ...
    output = model(input_ids, position_ids=position_ids, use_cache=True, output_hidden_states=debug, num_logits_to_keep=100)
    hiddens = []
...
@torch.no_grad()
def ulysses_prefill_and_decode(model, input_ids, position_ids, max_new_tokens = 12, stop_ids = [], debug=False, break_down=False):
    rank = dist.get_rank()
    world_size = dist.get_world_size()
    generated_ids = []
    # 1. prefill
...
@torch.no_grad()
def lring_prefill_and_decode(model, input_ids, position_ids, context_len, query_ids = None, max_new_tokens = 12, stop_ids = [], debug=False, break_down=False):
    ...
    if break_down:
        ...
        torch.cuda.synchronize()
        prefill_beg = time.time()
    output = model(input_ids[:, :context_len], query_ids=query_ids, position_ids=position_ids[:, :context_len], use_cache=True, output_hidden_states=debug, num_logits_to_keep=-1)
...
@torch.no_grad()
def star_prefill_and_decode(model, input_ids, position_ids, context_len, max_new_tokens = 12, stop_ids = [], debug=False, break_down=False):
    rank = dist.get_rank()
    world_size = dist.get_world_size()
    generated_ids = []
    # 1. prefill by ring attention
