...
from lring.utils import load_lring_model_tokenizer_dist, lring_prefill_and_decode_dist
import torch.distributed as dist
...
def load_model(
    ...
    model, tokenizer = load_lring_model_tokenizer_dist(model_path, locret_path=locret_path, nn=nn, np=np)
    # if dist.get_rank() == 0:
    #     breakpoint()
    # dist.barrier()
...
def main(
    ...
    with open(output_file, output_file_mode, encoding='utf-8', buffering=1) as fout:
        for input_sample in tqdm(input_data, total=len(input_data)):
            ...
            input_ids = tokenizer(input_str, return_tensors='pt', add_special_tokens=False).input_ids.to(model.device)
            
            
            # input_ids = torch.cat((input_ids[:, -100:], input_ids), dim=-1)
            # context_len += 100
            
            position_ids = torch.arange(input_ids.shape[-1], dtype=torch.long, device=model.device).unsqueeze(0)
            generated_ids = lring_prefill_and_decode_dist(model, input_ids, position_ids, context_len, max_new_tokens=max_new_tokens, stop_ids=stop_ids, nn=nn, np=np)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--nn_', type=int, default=1, help='number of nodes',
    )
    parser.add_argument(
        '--np_', type=int, default=1, help='nprocs per node',
    )
