...
def load_model(
    ...
    model, tokenizer = load_lring_model_tokenizer(model_path, locret_path=locret_path, batch=True)
    # if dist.get_rank() == 0:
    #     breakpoint()
    # dist.barrier()
...
def main(
    ...
    b = 14 # 1 is the largest available batch size
    l = 128*1024

    input_ids = torch.randint(0, tokenizer.vocab_size, (b, l)).to(model.device)
    query_ids = torch.randint(0, tokenizer.vocab_size, (b, 50)).to(model.device)
    position_ids = torch.arange(0, l).unsqueeze(0).repeat(b, 1).to(model.device)
    query_ids[:, -1] = 128009
    # breakpoint()
    tot_t = 0
    with torch.no_grad():
        for _ in tqdm(range(5)):
            output = model(input_ids, query_ids=query_ids, position_ids=position_ids, use_cache=True, num_logits_to_keep=-1)
        for _ in tqdm(range(5)):
            dist.barrier()
            beg = time.time()
            output = model(input_ids, query_ids=query_ids, position_ids=position_ids, use_cache=True, num_logits_to_keep=-1)
