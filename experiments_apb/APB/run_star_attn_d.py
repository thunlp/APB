...
from lring.utils import load_star_model_tokenizer_dist, star_prefill_and_decode_dist
import torch.distributed as dist
...
def load_model(
    ...
    model, tokenizer = load_star_model_tokenizer_dist(model_path, nn=nn, np=np)
...
def main(
    ...
    with open(output_file, output_file_mode, encoding='utf-8', buffering=1) as fout:
        for input_sample in tqdm(input_data, total=len(input_data)):
            ...
            generated_ids = star_prefill_and_decode_dist(model, input_ids, position_ids, context_len, max_new_tokens=max_new_tokens, stop_ids=stop_ids, nn=nn, np=np)
