...
from lring.utils import load_ulysses_model_tokenizer, ulysses_prefill_and_decode
import torch.distributed as dist
...
def load_model(
    ...
    model, tokenizer = load_ulysses_model_tokenizer(model_path)
    # if dist.get_rank() == 0:
    #     breakpoint()
    # dist.barrier()
...
def main(
    ...
    for l in [32, 64, 128, 256, 512, 1024]:
        ...
        with torch.no_grad():
            for _ in tqdm(range(5)):
                output = model(input_ids, position_ids=position_ids, num_logits_to_keep=-1)
            dist.barrier()
            # torch.cuda.synchronize()
            beg = time.time()
            output = model(input_ids, position_ids=position_ids, num_logits_to_keep=-1)
    ...
    with torch.no_grad():
        for _ in tqdm(range(10)):
            output = model(input_ids, position_ids=position_ids, breakdown=True)
        output = model(input_ids, position_ids=position_ids, breakdown=True)
