...
class VideoInferencePipeline:
    def __init__(self, model_path, dataset_path, json_file, rank, world_size):
        ...
        duration_group = int(os.getenv("DURATION_GROUP", 0))
        # 创建数据集和分布式采样器
        self.dataset = LongVideoBenchDataset(dataset_path, json_file, max_num_frames=64, duration_group=duration_group, insert_text=False)
