...
class VideoInferencePipeline:
    def __init__(self, model_path, dataset_path, json_file, rank, world_size):
        ...
        self.model = Qwen2_5_VLForConditionalGeneration.from_pretrained(
            model_path,
            # "/your_path_prefix/apb-v/checkpoints_apb_baseline_3b/checkpoint_3000",
        ...
        # self.dataset.data = self.dataset.data[475:] # TODO!!!
    ...
    def process_single_sample(self, data):
        ...
        dist.barrier()
        beg_t = time.time()
        generated_ids = self.model.star_generate(
        ...
        return output_text, choices, answer, end_t - beg_t, cnt_t
    ...
    def run_evaluation(self):
        ...
        for data in self.dataloader:
            # try:
            output_text, choices, answer, duration, cnt_t = self.process_single_sample(data)
            # except:
            #     pbar.update(1)
            #     pbar.set_postfix({"current_acc": "too short"})
            #     too_short += 1
            #     continue
        ...
        if self.rank == 0:
            pbar.close()
            print(f"avg. time: {(all_duration / all_cnt):.2f} s/req")
            print(local_correct, len(self.dataset) - too_short, too_short)
            return local_correct / (len(self.dataset) - too_short)
        else:
            return None
