...
def get_detail_result(data):
    ...
    for d in data:
        if not 'judge_result' in d:
            ...
            total_corr += d['pred'][0]==d['gt']

            print(d['video_path'], d['pred'][0]==d['gt'])
            # match = re.search(r'assistant\n([A-D])', d['pred'])
            # if match:
            #     pred = match.group(1)
            #     total_corr +=pred==d['gt']
            #     # breakpoint()
            #     print(d['video_path'], pred==d['gt'])
            #     d['judge_result'] = 1 if pred==d['gt'] else 0
            # else:
            #     print(d['video_path'], False)
            #     d['judge_result'] = 0
    ...
    print(f"current acc: {total_corr / len(data)}")
