...
from qwen2_5_vl.modeling_qwen2_5_vl_apbvz import Qwen2_5_VLForConditionalGeneration
...
class VideoQADataset(Dataset):
    def __init__(self, question_dict, video_dir, frames_num):
...
def run_inference(args, rank, world_size):
    """
    ...
    question_dict = [q for q in question_dict if q['type']=='cnt_insert1']
    # question_dict = [q for q in question_dict if q['video']=='./VNBench/v_NvRH4SoF09c_ord_edit1.mp4']
    qa_dataset = VideoQADataset(question_dict, args.video_dir, args.frames_num)
    ...
    for q_dict in tqdm(qa_dataloader):
        video_frames = q_dict['video_frames']
        ...
        query_str = "".join(qs) + "<|im_end|>\n<|im_start|>assistant\n"
        queries = processor.tokenizer(query_str, add_special_tokens=False, return_tensors='pt')['input_ids'].to(inputs.input_ids.device)
        ...
        with torch.inference_mode():
            if dist.get_rank() == 0:
                print(f"input_len: {inputs.input_ids.shape[-1]} video_size: {inputs.pixel_values_videos.shape}")
            output_ids = model.apbv_generate(
...
if __name__ == "__main__":
    ...
    run_inference(args, rank, world_size)