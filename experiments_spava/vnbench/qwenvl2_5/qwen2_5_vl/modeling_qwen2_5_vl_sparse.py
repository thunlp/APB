...
class Qwen2_5_VisionTransformerPretrainedModel(Qwen2_5_VLPreTrainedModel):
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor) -> torch.Tensor:
        ...
        sparsity = 0.1
        fw, fh = grid_thw[0, 1:]
        hidden_states = hidden_states.reshape(-1, fw * fh, hidden_states.shape[-1])
        delta = hidden_states[1:] - hidden_states[:-1]
        metrics = delta.abs().max(dim=-1).values
        indices = metrics.topk(dim=-1, k=int(sparsity * metrics.shape[-1])).indices
        mask = torch.zeros_like(delta)
        batch_size = mask.shape[0]
        batch_indices = torch.arange(batch_size).unsqueeze(1)
        mask[batch_indices, indices, :] = 1
        delta = delta * mask
        cumsum_delta = delta.cumsum(dim=0)
        hidden_states_ = hidden_states[0] + cumsum_delta
        hidden_states = torch.cat((hidden_states[:1], hidden_states_), dim=0)
