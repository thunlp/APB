...
class Qwen2_5_VLVisionBlock(nn.Module):
    ...
    def forward(
        ...
        batch_seqlen = (sparsity * batch_seqlen).to(torch.int32)
        batch_seqlen = torch.cat([torch.tensor([0], dtype=batch_seqlen.dtype, device=batch_seqlen.device), batch_seqlen])
        cu_seqlens_new = torch.cumsum(batch_seqlen, -1).to(torch.int32)
        _indices = torch.tensor(random.sample(range(hidden_states.shape[0]), cu_seqlens_new[-1]), device=hidden_states.device)
        indices = _indices.unsqueeze(-1).repeat(1, hidden_states.shape[-1])
        new_hs = hidden_states.gather(0, indices)
        indices_pe = _indices.unsqueeze(-1).repeat(1, position_embeddings[0].shape[-1])
        new_ps = (position_embeddings[0].gather(0, indices_pe), position_embeddings[1].gather(0, indices_pe))

        new_hs = new_hs + self.attn(
            self.norm1(new_hs),
            cu_seqlens=cu_seqlens_new,
            rotary_pos_emb=None,
            position_embeddings=new_ps
        )
        ...
        new_hs = hidden_states.gather(0, indices)
        new_hs = new_hs + self.mlp(self.norm2(new_hs))
        hidden_states.scatter_(0, indices, new_hs)
        return hidden_states
